...
> ⛔ **업무망에서 이 레포에 커밋·push 하지 마라.** 공개 레포다.
> 여기는 클론해서 스킬·문서를 **꺼내가는 곳**이고, 작업은 사내 enterprise 레포에서 한다.
> 자세한 건 섹션 3의 "이 포크는 어디서 쓰는 물건인가".
...
### 🔑 이 포크는 어디서 쓰는 물건인가 — 먼저 읽어라

**이 포크는 "회사 작업이 의존하는 레포"가 아니다. 이식 작업대 겸 배송 수단이다.**
이걸 헷갈리면 유출이 난다.

```
집 (인터넷 있음)                        업무망
──────────────────────                 ─────────────────────────
upstream ──pull──▶ 이 포크              이 포크를 클론 (읽기 전용)
                   dc-port 에서 이식          │
                   검사기로 검증               ▼
                        │                enterprise 레포에서 작업
                        └───────────────▶ (여기가 배포물의 진짜 집)

                   ⛔ 업무망 → 이 포크로 push 절대 금지
```

| | 어디서 | 왜 |
|---|---|---|
| upstream 당기기 · 재이식 · 검사기 | **집** | 회사 정보가 섞일 일이 없고, mcp-atlassian 소스로 검증할 수 있다 |
| 클론해서 스킬·문서 꺼내기 | **업무망** | GitHub 이 뚫려 있다 |
| 실제 작업·사내 값 채우기 | **업무망의 enterprise 레포** | 여기에만 회사 정보가 들어간다 |

**⛔ 업무망에서 이 포크로 push 하지 마라.**
- 이 포크는 **공개 레포**다. 회사 파생 작업을 밀어넣으면 그 자체가 유출이다.
- 회사 자산이 개인 계정에 남는다. 거버넌스 문제다.
- **클론해서 내용물을 꺼냈으면 끈을 끊어라.** 배송 수단이지 의존 대상이 아니다.

**upstream(`atlassian/atlassian-mcp-server`)을 업무망에서 클론할 이유는 없다.**
거기엔 Cloud 전용 원본만 있다. 클론하면 이식을 처음부터 다시 해야 한다.
upstream 이력이 필요하면 이 포크 안에 이미 다 있다 (`git log main`).

> **왜 이 구조인가:** 배포물은 enterprise 레포에서 어차피 얼어붙는다.
> upstream 신선도는 이 포크까지만 온다. 그게 한계이자 **의도**다 —
> 포크를 배포 경로에 끼워넣으면 4단계 복사 체인이 되고, 업무망에서 push 하고 싶은 유혹이 생긴다.
> **포크는 집에 두고, 업무망은 클론만 한다.**
...
**업무망에서는 애초에 이 레포에 커밋할 일이 없다.** 클론해서 꺼내가는 곳이다 (섹션 3).
작업물은 사내 enterprise 레포로 간다. **이 규칙만 지키면 아래는 신경 쓸 일이 없다.**

집에서 이 레포를 손볼 때(재이식 등)만 아래가 적용된다. 이 레포는 **공개 포크**다
(`github.com/cruellaDev/atlassian-mcp-server`, PUBLIC).
