...
> 여기 적힌 사실들은 **인터넷이 있는 환경에서 실제로 소스를 읽고 실행해서 확인한 것**이다.
> 당신이 폐쇄망에 있다면 **이것들을 다시 확인할 방법이 없다.** 그래서 여기 적어둔 것이다.
...
## 6. 검증 방법 — 폐쇄망에서 할 수 있는 것

인터넷이 없으면 소스를 못 본다. **실제 툴 호출로만 검증한다.**

1. `/mcp` → 서버 connected + 툴 목록 확인. **4번 표를 이 목록과 대조하라.**
2. 읽기 전용 스킬부터: 사용자에게 "우리 문서에서 [아는 주제] 찾아줘" 요청
   → `search-company-knowledge` 발동 + `jira_search`/`confluence_search` 호출되면 성공
3. 툴 이름 에러가 나면 → 치환 누락. **에러 메시지에 찾던 이름이 그대로 찍힌다.** 그게 단서다.

**두 번 고쳐서 안 되면 멈춰라.** 추측으로 세 번째 수정을 하지 말고 에러 원문을 사람에게 보여줘라.

### 이 레포에서 쓸 수 있는 자체 검사

인터넷 없이도 돈다 (mcp-atlassian 소스가 있을 때만):
```bash
# 스킬이 부르는 툴이 실재하는지
python3 -c "..."   # HANDOVER 작성 시 사용한 AST 대조 스크립트. 소스 없으면 /mcp 목록으로 손 대조
```
소스가 없으면 **`/mcp` 목록과 눈으로 대조**하는 수밖에 없다. 그게 이 문서 4번 표가 존재하는 이유다.
...
> "MCP 서버가 `.mcp.json`에 뭐로 등록돼 있나요? (PAT는 가리고 이름만)"

실제 호출명은 **`mcp__<서버이름>__<툴이름>`** 이다.
`mcp-atlassian`으로 등록 → `mcp__mcp-atlassian__jira_search`. **prefix를 모르면 아무것도 못 한다.**
